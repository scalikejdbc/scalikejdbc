    ...
    try {
      DB autoCommit {
        implicit s =>
          ...
          {
            val groups: List[Group] = SQL("select g.id as g_id, m.id as m_id, s.id as s_id " +
    ...
    try {
      DB autoCommit {
        implicit s =>
          ...
          {
            val groups: List[Group] = SQL("select g.id as g_id, g.owner_id as g_owner_id, o.id as o_id, m.id as m_id, s.id as s_id " +
              " from groups_" + suffix + " g " +
