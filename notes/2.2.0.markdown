...
Since this version, library users can define and another transaction boundary instead of Exception when using localTx blocks.
...
Built-in type class instances are `Try`, `Either` and `Future`. You can use them by importing `import scalikejdbc.TxBoundary,***._`.
