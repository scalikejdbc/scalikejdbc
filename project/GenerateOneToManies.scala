...
s"          to$i.map(t => Vector(t)).getOrElse(Vector.empty)"
  ...
  private[scalikejdbc] def toTraversable(session: DBSession, sql: String, params: Seq[_], zExtractor: (A, $seq) => Z): Traversable[Z] = {
    val attributesSwitcher = createDBSessionAttributesSwitcher()
    DBSessionWrapper(session, attributesSwitcher).foldLeft(statement, parameters: _*)(LinkedHashMap[A, ($seq)]())(processResultSet).map {
...
class OneToManies${n}SQL[A, $bs, E <: WithExtractor, Z](
  ...
  override val rawParameters: Seq[Any])(val one: WrappedResultSet => A)($resultSetToOptions)(val zExtractor: (A, $seq) => Z)
...
class OneToManies${n}SQLToOption[A, $bs, E <: WithExtractor, Z](
  override val statement: String,
  override val rawParameters: Seq[Any])(val one: WrappedResultSet => A)($resultSetToOptions)(val zExtractor: (A, $seq) => Z)(val isSingle: Boolean = true)
    extends SQL[Z, E](statement, rawParameters)(SQL.noExtractor[Z]("one-to-many extractor(one(RS => A).toMany(RS => Option[B1])) is specified, use #map((A,B) =>Z) instead."))
