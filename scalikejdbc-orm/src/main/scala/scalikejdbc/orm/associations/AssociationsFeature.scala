...
trait AssociationsFeature[Entity]
  ...
  private def unshiftJoinDefinition(
    newOne: JoinDefinition[_],
    definitions: mutable.LinkedHashSet[JoinDefinition[_]]
  ): mutable.LinkedHashSet[JoinDefinition[_]] = {
  ...
  def createJoinDefinition(
    joinType: JoinType,
    left: (AssociationsFeature[_], Alias[_]),
    right: (AssociationsFeature[_], Alias[_]),
    on: SQLSyntax
  ...
  def joinWithDefaults[Left](
    ...
    right: AssociationsFeature[_],
    on: (Alias[Left], Alias[_]) => SQLSyntax
  ): JoinDefinition[Entity] = {
  ...
  def join(
    right: (AssociationsFeature[_], Alias[_]),
    on: (Alias[Entity], Alias[_]) => SQLSyntax
  ): JoinDefinition[Entity] = {
    innerJoin(right, on)
  }
  def join[Left](
  ...
  def leftJoinWithDefaults(
    left: AssociationsFeature[_],
    right: AssociationsFeature[_],
    on: (Alias[_], Alias[_]) => SQLSyntax
  ): JoinDefinition[_] = {
    createJoinDefinition(
      LeftOuterJoin,
      left -> left.defaultAlias,
  ...
  def belongsToWithAlias[A](
    right: (AssociationsWithIdFeature[_, A], Alias[A]),
    merge: (Entity, Option[A]) => Entity
  ): BelongsToAssociation[Entity] = {
  ...
  def hasManyWithFk[M](
    ...
    val joinDef = leftJoin(
      this -> this.defaultAlias,
      many,
      on.asInstanceOf[(Alias[_], Alias[_]) => SQLSyntax]
    )
  ...
  def hasManyThrough[M2](
    through: AssociationsFeature[_],
    many: AssociationsWithIdFeature[_, M2],
  ...
  def hasManyThrough[M1, M2](
    ...
    val joinDef1 = leftJoin(
      through,
      throughOn.asInstanceOf[(Alias[_], Alias[_]) => SQLSyntax]
    )
  ...
  def extractWithAssociations(
    ...
    if (enabledJoinDefinitions.isEmpty) {
      ...
      if (enabledOneToManyExtractors.size == 1) {
        ...
        val sql: OneToManies2SQL[Entity, _, _, HasExtractor, Entity] =
          oneExtractedSql
            .toManies(
        ...
        val Seq(
          ...
          enabledOneToManyExtractors.toSeq
        val sql: OneToManies3SQL[Entity, _, _, _, HasExtractor, Entity] =
          oneExtractedSql
            .toManies(
              to1 = rs => extractHasMany(ex1, rs),
              to2 = rs => extractHasMany(ex2, rs),
        ...
        val sql: OneToManies4SQL[Entity, _, _, _, _, HasExtractor, Entity] =
          oneExtractedSql
            .toManies(
              to1 = rs => extractHasMany(ex1, rs),
              to2 = rs => extractHasMany(ex2, rs),
        ...
        val sql: OneToManies5SQL[Entity, _, _, _, _, _, HasExtractor, Entity] =
          oneExtractedSql
            .toManies(
              to1 = rs => extractHasMany(ex1, rs),
              to2 = rs => extractHasMany(ex2, rs),
        ...
        val sql
          : OneToManies6SQL[Entity, _, _, _, _, _, _, HasExtractor, Entity] =
          oneExtractedSql
            .toManies(
              to1 = rs => extractHasMany(ex1, rs),
              to2 = rs => extractHasMany(ex2, rs),
        ...
        val sql
          : OneToManies7SQL[Entity, _, _, _, _, _, _, _, HasExtractor, Entity] =
          oneExtractedSql
            .toManies(
              to1 = rs => extractHasMany(ex1, rs),
              to2 = rs => extractHasMany(ex2, rs),
