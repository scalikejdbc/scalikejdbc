...
trait SelectDynamicMacro[A] { self: SQLSyntaxSupportFeature#SQLSyntaxProvider[A] =>
  ...
  inline def select[E](ref: SQLSyntaxSupportFeature#SQLSyntaxProvider[A], inline name: String): SQLSyntax =
    ${SelectDynamicMacroImpl.selectImpl[E]('ref, 'name)}
}

object SelectDynamicMacroImpl {
  def selectImpl[E: Type](ref: Expr[SQLSyntaxSupportFeature#SQLSyntaxProvider[_]], name: Expr[String])(using quotes: Quotes): Expr[SQLSyntax] = {
    ...
    name.value.foreach { _name =>
      if (expectedNames.nonEmpty && !expectedNames.contains(_name)) {
        report.error(
          s"${typeSymbol.fullName}#${_name} not found. Expected fields are ${expectedNames.mkString("#", ", #", "")}",
          name.asTerm.pos
        )
      }
    }
