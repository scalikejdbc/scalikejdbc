...
class QueryInterfaceSpec extends FlatSpec with Matchers with DBSettings with SQLInterpolation {
  ...
  case class Order(id: Int, productId: Int, accountId: Option[Int], createdAt: ZonedDateTime, product: Option[Product] = None, account: Option[Account] = None)
  ...
  object Order extends SQLSyntaxSupport[Order] {
    ...
    def apply(o: ResultName[Order])(rs: WrappedResultSet): Order = {
      new Order(rs.int(o.id), rs.int(o.productId), rs.intOpt(o.accountId), rs.zonedDateTime(o.createdAt))
    }
  ...
  object TimeHolder extends SQLSyntaxSupport[TimeHolder] {
    ...
    def apply(p: ResultName[TimeHolder])(rs: WrappedResultSet): TimeHolder = new TimeHolder(rs.int(p.id), rs.zonedDateTime(p.time))
  }
    ...
    try {
      ...
      DB localTx { implicit s =>
        ...
        Seq(
          ...
          insert.into(Order).values(26, 2, None, ZonedDateTime.now)).foreach(sql => applyUpdate(sql))
  ...
  it should "convert timeZone" in {
    ...
    val time = ZonedDateTime.of(2016, 1, 9, 2, 43, 42, 0, ZoneId.of("Asia/Tokyo"))
