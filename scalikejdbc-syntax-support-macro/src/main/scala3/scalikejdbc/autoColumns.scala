...
object autoColumns {
  ...
  def apply_impl[A](excludes:Expr[Seq[String]])(using quotes:Quotes)(using Type[A]):Expr[Seq[String]] = {
    ...
    // this.nameConverters
    val nameConverters = Select.unique(thi, "nameConverters").asExprOf[Map[String,String]]
    ...
    val useSnakeCaseColumnName = Select.unique(thi, "useSnakeCaseColumnName").asExprOf[Boolean]
