...
object autoConstruct {
  ...
  def applySyntaxProvider_impl[A](rs:Expr[WrappedResultSet], sp:Expr[SyntaxProvider[A]], excludes:Expr[Seq[String]])(using quotes:Quotes)(using t:Type[A]):Expr[A] = {
    applySyntaxProvider_impl(rs,'{$sp.resultName}, excludes)
  }
  inline def apply[A](rs: WrappedResultSet, sp: SyntaxProvider[A], excludes: String*):A = ${applySyntaxProvider_impl('rs,'sp, 'excludes)}
